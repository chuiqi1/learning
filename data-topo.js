module.exports = {
  ...
  nodes: [
    ...
    },
    {
      ...
      content: "覆叠空间 p: X̃→X 是满足局部平凡化条件的连续满射：对每个 x∈X，存在邻域 U 使得 p⁻¹(U) 同胚于 U×F（F 为离散空间）。覆叠空间的纤维 p⁻¹(x) 的基数称为覆叠次数。覆叠空间最重要的性质是道路提升定理：X 中任一道路可唯一提升到 X̃ 中（给定起点）；以及同伦提升定理：道路的同伦也可提升。覆叠空间与基本群通过 Galois 对应紧密关联：令 H=p_*π₁(X̃)。一般连通覆叠的覆叠变换群同构于正规化子商 N(H)/H；只有 H 为正规子群（即正规覆叠）时，才简化为 π₁(X)/H。",
    ...
    },
    {
      ...
      theorems: [
        { name: "覆叠空间分类定理", statement: "设 X 道路连通、局部道路连通且半局部单连通。则连通覆叠空间 p: X̃→X 的等价类与 π₁(X,x₀) 的子群共轭类一一对应；带基点的连通覆叠等价类与具体子群一一对应。", proof: "给定子群H⊂π₁(X)，取万有覆叠X̃_univ，并让H作为覆叠变换群的子群作用，轨道商X̃_univ/H就是对应覆叠。反之，给定带基点覆叠，取子群p_*(π₁(X̃))。改变纤维中的基点会把该子群共轭，因此忘掉基点后对应的是共轭类。" },
        { name: "万有覆叠的存在性", statement: "设 X 道路连通且局部道路连通，则 X 半局部单连通当且仅当 X 存在单连通覆叠；这样的单连通覆叠称为万有覆叠。", proof: "构造：取X中以x₀为起点、相对端点同伦的道路类集合，赋以由局部延长道路得到的基本开集拓扑，投影取道路终点p([γ])=γ(1)。半局部单连通性保证每个小邻域的逆像分裂为互不相交的覆盖片。反向若有单连通覆叠，则均匀覆盖邻域中的环路提升为闭环路并可在覆叠空间中收缩，所以其在X中也零伦。" }
      ],
    ...
    },
    {
      ...
      theorems: [
        ...
        { name: "初等塌缩诱导形变收缩", statement: "设有限单纯复形 K 中的 n 单形 σ 有一个自由面 τ（τ 不属于任何其他 n 单形）。删去 σ 与 τ 的开单形得到子复形 L，则 |L| 是 |K| 的强形变收缩核；有限次初等塌缩保持同伦型。", proof: "把 σ 写成重心坐标 λ₀,…,λₙ，并设 τ 是 λ₀=0 的面。令 m=min(λ₁,…,λₙ)，把每个 λᵢ(i≥1) 改为 λᵢ-m，把 λ₀ 改为 λ₀+nm。新坐标非负且和为1，并至少有一个 λᵢ(i≥1) 为0，所以像落在 σ 除去 τ 内部后的其余面上；该部分原本就有 m=0，因而逐点固定。用直线同伦连接恒等映射与此收缩，再在 σ 外取恒等映射，由粘贴引理得到强形变收缩。" }
      ],
    ...
    // ==================== Ch2: 同调论 ====================
    {
      ...
      examples: [
        ...
        { title: "射影平面 RP^2 的三角剖分", content: "RP^2 的最小三角剖分有6个顶点、15条边、10个三角形；其整数同调 H_1(RP^2)≅Z/2 含有扭转。" }
      ],
      theorems: [
